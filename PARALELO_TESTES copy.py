...
def modelo_intervencao(df1, coluna_data, coluna_valor, data_intervencao, nome_arquivo, title, eixo_y, p=1, d=1, q=1):
    ...
    # Ajustar o modelo ARIMA com os parâmetros recebidos
    modelo = sm.tsa.ARIMA(df[coluna_valor], order=(p, d, q), exog=df[['intervencao']])
    print(f"\nModelo ARIMA ({p}, {d}, {q}) será usado.")
    ...
    modelo = sm.tsa.SARIMAX(df[coluna_valor], order=(p, d, q), seasonal_order=(P, D, Q, S), exog=df[['intervencao']])
    
    
    resultado = modelo.fit()
    print(f"\nModelo SARIMAX ({p}, {d}, {q}) será usado.")
    
    # Imprimir resumo do modelo
    print(resultado.summary())
    
    # Fazer previsões
    previsao = resultado.predict(start=2, end=len(df)-1, exog=df[['intervencao']].iloc[2:])  # start ajustado
    
    # Plotar os resultados
    plt.figure(figsize=(10, 6))
    plt.plot(df.index, df[coluna_valor], label='Série Original')
    plt.plot(df.index[2:], previsao, label='Previsão ARIMA com Intervenção', color='red')
...
def otimizar_arima(df, coluna, p_values, d_values, q_values, title):
    """
    Função para otimizar os parâmetros (p, d, q) de um modelo ARIMA
    baseado em uma série temporal.
    
    Parâmetros:
    df: DataFrame com a série temporal
    coluna: nome da coluna do DataFrame com a série temporal
    p_values: Lista de valores possíveis para p (parte autorregressiva)
    d_values: Lista de valores possíveis para d (ordem de diferenciação)
    q_values: Lista de valores possíveis para q (parte de média móvel)
    
    Retorna:
    Os melhores valores de p, d e q que minimizam o AIC e o BIC.
    """
    
    # Definir os critérios de informação
    resultados = []
    
    # Combinações de parâmetros
    pdq = list(itertools.product(p_values, d_values, q_values))
    
    # Loop através das combinações de parâmetros
    for param in pdq:
        try:
            # Ajustar o modelo ARIMA
            modelo_arima = sm.tsa.ARIMA(df[coluna], order=param)
            modelo_arima_fit = modelo_arima.fit()
            
            # Guardar os resultados do AIC e BIC
            resultados.append({
                'param': param,
                'AIC': modelo_arima_fit.aic,
                'BIC': modelo_arima_fit.bic
            })
            
            print(f'Modelo ARIMA{param} - AIC: {modelo_arima_fit.aic}, BIC: {modelo_arima_fit.bic}')
    ...
    # Identificar o melhor modelo (menor AIC e BIC)
    melhor_aic = resultados_df.loc[resultados_df['AIC'].idxmin()]
    melhor_bic = resultados_df.loc[resultados_df['BIC'].idxmin()]
    
    print(f'\nMelhor modelo baseado no AIC: ARIMA{melhor_aic["param"]} - AIC: {melhor_aic["AIC"]}')
    print(f'Melhor modelo baseado no BIC: ARIMA{melhor_bic["param"]} - BIC: {melhor_bic["BIC"]}')
    
    # Transformar as tuplas dos parâmetros (p, d, q) em strings para plotagem
    resultados_df['param_str'] = resultados_df['param'].apply(lambda x: f'{x}')
    
    # Plotar a evolução do AIC e BIC
    plt.figure(figsize=(10, 6))
    plt.plot(resultados_df['param_str'], resultados_df['AIC'], label='AIC', marker='o')
    plt.plot(resultados_df['param_str'], resultados_df['BIC'], label='BIC', marker='x')
    plt.xlabel('Parâmetros (p,d,q)')
    plt.ylabel('Valores de AIC e BIC')
    title += '- Evolução do AIC e BIC para diferentes modelos ARIMA'
    plt.title(title)
    plt.legend()
    ...
    # Retornar os melhores parâmetros para AIC e BIC
    return {
        'melhor_aic': {
            'param': melhor_aic['param'],
            'AIC': melhor_aic['AIC']
        },
        'melhor_bic': {
            'param': melhor_bic['param'],
            'BIC': melhor_bic['BIC']
        }
    }
...
fim_pandemia='2022-05-23'
datas_marcadas = [inicio_pandemia, fim_pandemia] 
...
resultados = otimizar_arima(df_fipezap, 'Preço Médio Venda', p_values, d_values, q_values, title)
p, d, q = resultados['melhor_aic']['param']

print("Melhor combinação para AIC:")
print(resultados['melhor_aic'])

print("Melhor combinação para BIC:")
print(resultados['melhor_bic'])

resultado = modelo_intervencao(df_sbpe_33_pib, 'Data', 'indices_imobiliario_pib_br',inicio_pandemia,nome_arquivo,title,eixo_y,p,1,1)
...
resultados = otimizar_arima(df_sbpe_09_modalidade_agrupada, 'total_contratado', p_values, d_values, q_values, title)
p, d, q = resultados['melhor_aic']['param']
...
resultado = modelo_intervencao(df_sbpe_09_modalidade_agrupada, 'Data', 'total_contratado', inicio_pandemia,nome_arquivo,title,eixo_y,1,1,3)
...
resultados = otimizar_arima(df_sbpe_27_tipoimovel, 'soma_imoveis', p_values, d_values, q_values, title)
p, d, q = resultados['melhor_aic']['param']
...
resultado = modelo_intervencao(df_sbpe_27_tipoimovel, 'Data', 'soma_imoveis', inicio_pandemia,nome_arquivo,title,eixo_y,0,1,1)
...
resultados = otimizar_arima(df_sbpe_31_valor_de_compra, 'imoveis_valor_compra_br', p_values, d_values, q_values, title)
p, d, q = resultados['melhor_aic']['param']
...
resultado = modelo_intervencao(df_sbpe_31_valor_de_compra, 'Data', 'imoveis_valor_compra_br', inicio_pandemia,nome_arquivo,title,eixo_y,1,1,1)
...
resultado_int = modelo_intervencao(df_fipezap, 'Data', 'Preço Médio Venda', inicio_pandemia,nome_arquivo,title,eixo_y,1,1,3)
...
resultados = otimizar_arima(df_fipezap, 'Preço Médio Aluguel', p_values, d_values, q_values, title)
print("Melhor combinação para AIC:")
print(resultados['melhor_aic'])
...
resultado = modelo_intervencao(df_fipezap, 'Data', 'Preço Médio Aluguel', inicio_pandemia,nome_arquivo,title,eixo_y,1,1,3)



####### Análise do renda da população
...
resultados = otimizar_arima(df_pnad, 'Renda Média', p_values, d_values, q_values, title)
...
resultado = modelo_intervencao(df_pnad, 'Data', 'Renda Média',inicio_pandemia,nome_arquivo,title,eixo_y,1,1,3)
"""
...
resultados = otimizar_arima(df_cub, 'Global', p_values, d_values, q_values, title)
p, d, q = resultados['melhor_aic']['param']
...
resultado = modelo_intervencao(df_cub, 'Data', 'Global', inicio_pandemia,nome_arquivo,title,eixo_y,p,d,q)
